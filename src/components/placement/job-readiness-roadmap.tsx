...
import { CheckCircle2, Circle, Clock, Bot, Plus } from "lucide-react";
...
export function JobReadinessRoadmap({ targetRole, tasks, onOpenAgent }: JobReadinessRoadmapProps) {
  ...
  return (
    <Card className="p-6 sm:p-8">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-5 mb-6">
        <div>
          <SectionHeader
      ...
      <div className="relative space-y-8 before:absolute before:left-4 before:top-3 before:bottom-3 before:w-0.5 before:bg-border/60">
        {stages.map((stage) => {
            ...
            <div key={stage.number} className="relative pl-10">
              ...
              <div className="rounded-xl border border-border/80 bg-background p-4 sm:p-5">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h4 className="text-base font-bold text-text">{stage.title}</h4>
                  ...
                  <Button
                    ...
                    className="shrink-0 text-xs hover:border-primary hover:text-primary"
                  >
                ...
                {stageTasks.length > 0 ? (
                  <div className="mt-4 space-y-2 border-t border-border/50 pt-3">
                    {stageTasks.map((task) => (
                      <div
                        key={task.id}
                        className="flex items-center justify-between gap-3 rounded-lg border border-border/40 bg-card p-3 text-xs"
                      >
